...
export function B2BProspectCard({ record, decision, comparing, onOpen, onDecision, onCompare }: B2BProspectCardProps) {
  return (
    <article className={decision === 'Shortlisted' ? 'prospect-card is-shortlisted' : decision === 'Not a fit' ? 'prospect-card is-not-fit' : 'prospect-card'}>
      <div className="prospect-card__summary">
        <div className="prospect-card__identity">
          <span className="company-avatar"><Building2 /></span>
          <div className="prospect-card__identity-copy">
            <div className="title-row"><h3>{record.companyName}</h3>{record.tier && <span className="tier-badge">{record.tier}</span>}{record.warnings.length > 0 && <span className="warning-count" title={`${record.warnings.length} data-quality notes`}>! {record.warnings.length}</span>}</div>
            <p>{[record.category, record.subcategory].filter(Boolean).join(' · ') || 'Category not provided'}</p>
            <div className="prospect-card__meta">
              <span><MapPin size={13} />{record.location || 'Location not provided'}</span>
              {record.dateAdded && <span><CalendarDays size={13} />{record.dateAdded}</span>}
            </div>
          </div>
          <div className="total-score"><strong>{record.sourceTotalScore ?? '—'}</strong><small>Total score</small></div>
        </div>
        <div className="prospect-card__scores" aria-label="Scoring dimensions">
        ...
        <div className="prospect-card__contact">
          <div className="prospect-card__contact-person">
            <UserRound />
            <div><strong>{record.decisionMaker || 'Decision maker not provided'}</strong><span>{record.role || 'Role not provided'}</span></div>
          </div>
          <div className="contact-links" aria-label="Prospect contact options"><ExternalButton href={record.website} label={`Open ${record.companyName} website`} icon={ExternalLink} /><ExternalButton href={record.linkedInProfile} label={`Open ${record.decisionMaker || record.companyName} on LinkedIn`} icon={Linkedin} />{record.emails[0] && <a className="link-icon" href={`mailto:${record.emails[0]}`} aria-label={`Email ${record.emails[0]}`} title={record.emails.join('; ')}><Mail size={15} /></a>}</div>
        </div>
      </div>
      <div className="prospect-card__insights">
        <div className="prospect-card__insight"><small>Observed activity</small><p>{record.observedCampaignActivity || 'No observed activity provided.'}</p></div>
        <div className="prospect-card__insight hypothesis"><small>Hypothesis</small><p>{record.hypothesizedPain || 'No pain hypothesis provided.'}</p></div>
        <div className="prospect-card__insight"><small>Outreach angle</small><p>{record.outreachAngle || 'No outreach angle provided.'}</p></div>
      </div>
